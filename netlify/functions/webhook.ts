// src/index.ts
import { Telegraf } from 'telegraf';
// import { resolve } from 'path'; // resolve не нужен в этом коде

// !!! ВАЖНО: Замените эти заглушки на свои данные !!!
// (Вы уже подставили свои ID, это просто напоминание)
const BOT_TOKEN = process.env.BOT_TOKEN || '8232714512:AAFegQAxLFgr6c-DbnaxyjbwOeCpoutF8h0'; 
const ADMIN_CHAT_ID = process.env.ADMIN_CHAT_ID || '476032497';
// ----------------------------------------------------

const TELEGRAM_API = `https://api.telegram.org/bot${BOT_TOKEN}`;

// Статусы сессии для каждого пользователя
interface Session {
    step: number;
    data: {
        name?: string;
        idea?: string;
        email?: string;
    }
}
const sessions: { [key: number]: Session } = {}; 

// Регулярное выражение для базовой проверки Email
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

/**
 * Отправляет сообщение пользователю через Telegram API.
 */
async function sendMessage(chatId: number, text: string, options: any = {}) {
    const url = `${TELEGRAM_API}/sendMessage`;
    const payload = {
        chat_id: chatId,
        text: text,
        parse_mode: 'Markdown',
        ...options
    };
    
    try {
        // В Serverless средах 'fetch' обычно глобально доступен
        await fetch(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        });
    } catch (error) {
        console.error('Error sending message:', error);
    }
}

/**
 * Форматирует и отправляет уведомление администратору.
 */
async function sendAdminNotification(data: Session['data']) {
    const notificationMessage = 
        `*🚨 НОВАЯ ЗАЯВКА PRIME STUDIO DEV 🚨*\n` +
        `----------------------------------------\n` +
        `👤 *Имя/Ник:* ${data.name || 'Не указано'}\n` +
        `📧 *Контакты:* ${data.email || 'Не указано'}\n` + // Изменено на "Контакты" для соответствия шагу 3
        `💡 *Идея проекта:*\n${data.idea || 'Не указано'}\n` +
        `----------------------------------------\n` +
        `_Свяжитесь с клиентом в течение 24 часов._`;
            
    // Приведение к числу, так как ADMIN_CHAT_ID — это строка
    await sendMessage(parseInt(ADMIN_CHAT_ID), notificationMessage); 
}


/**
 * Основная логика обработки входящего сообщения.
 * Принимает сырой update, независимо от источника (Webhook или Long Polling).
 */
async function handleUpdate(update: any) {
    if (!update.message) return;

    const chatId = update.message.chat.id;
    const text = update.message.text ? update.message.text.trim() : '';
    const userId = update.message.from.id;

    // Инициализация или получение текущей сессии
    if (!sessions[userId]) {
        sessions[userId] = { step: 0, data: {} };
    }

    let session = sessions[userId];
    let responseText = '';

    // --- Сброс и начало сессии ---
    if (text === '/start') {
        session.step = 1;
        session.data = {};
        responseText = 
            `*Привет!* На связи помощник команды Prime Studio Dev. ` + 
            `Я помогу быстро собрать всю необходимую информацию по Вашему проекту. Давайте начнем.\n\n` + 
            `*Шаг 1 из 3:* Для начала, как я могу к Вам обращаться? Пожалуйста, введите Ваше имя.`;
        await sendMessage(chatId, responseText);
        return;
    }
    
    if (session.step === 0) {
        responseText = "Для начала работы, пожалуйста, введите команду /start";
        await sendMessage(chatId, responseText);
        return;
    }


    // --- Логика пошагового сбора данных и валидации ---

    switch (session.step) {
        case 1: // Ожидание Имени
            session.data.name = text;
            session.step = 2;
            responseText = 
                `*Шаг 2 из 3:* Отлично, ${session.data.name}, спасибо! Теперь перейдем к проекту. Опишите Вашу основную идею. ` +
                `Пожалуйста, включите следующие детали:\n` +
                `1. Название проекта\n` +
                `2. Ключевые функции\n` +
                `3. Какая цель у Вашего проекта`;
            break;

        case 2: // Ожидание Идеи
            session.data.idea = text;
            session.step = 3;
            responseText = 
                `*Шаг 3 из 3 (последний):* Понял! И финальный штрих: ` +
                `пожалуйста, укажите Ваши контактные данные в удобном для Вас формате ` +
                `(*Email*, *Telegram*, *WhatsApp*). Мы используем их для официального ответа на Ваш запрос.`;
            break;

        case 3: // Ожидание Контактов и завершение (удалена строгая проверка email)
            // Валидация убрана, так как вы ждете разные контакты (email, telegram, whatsapp)
            // Мы сохраняем весь ввод пользователя как контакты.
            session.data.email = text; // Используем поле email для сохранения контактов
            session.step = 4; // Завершение сессии
            
            // 1. Отправляем уведомление вам (ADMIN_CHAT_ID)
            await sendAdminNotification(session.data);
            
            // 2. Отправляем сообщение пользователю
            responseText = 
                `*✅ Готово!* Ваша заявка успешно принята. Команда Prime Studio Dev ознакомится с деталями и ` + 
                `свяжется с Вами лично в течение *24 часов*. Спасибо за интерес к совместной работе!`;
            
            // Удаляем сессию
            delete sessions[userId];
            break;

        default:
            responseText = "Сессия завершена. Начните новую командой /start";
            break;
    }

    if (session.step !== 4 && session.step !== 0) {
        await sendMessage(chatId, responseText);
    }
}

// ====================================================================
// --- ДУАЛЬНАЯ ЛОГИКА ЗАПУСКА (Dev/Prod) ---
// ====================================================================

// Проверяем, если мы находимся в локальной среде разработки 
// (например, при запуске через ts-node, где VERCEL не определен)
if (!process.env.VERCEL) {
    
    // 1. Инициализируем Telegraf
    const bot = new Telegraf(BOT_TOKEN);
    
    // 2. Настраиваем обработчик для всех текстовых сообщений
    bot.on('text', async (ctx) => {
        // Форматируем сообщение Telegraf в сырой update для нашей функции handleUpdate
        const update = { message: ctx.message }; 
        await handleUpdate(update);
    });

    // 3. Запускаем бота в режиме Long Polling
    bot.launch()
      .then(() => console.log('✅ Bot запущен в режиме Long Polling для разработки. Тестируйте в Telegram.'))
      .catch(err => console.error('❌ Ошибка запуска бота (Long Polling):', err));

} else {
    // --- Обработчик для Serverless-функции (Production Webhook) ---
    // (Этот блок выполнится только после деплоя на Vercel/Netlify)
    
    module.exports = async (req: any, res: any) => {
        if (req.method === 'POST' && req.body) {
            await handleUpdate(req.body);
            res.status(200).send('OK'); 
        } else {
            res.status(200).send('Bot is running and ready for webhooks.');
        }
    };
}
// Убираем старый Telegraf import, так как он не использовался в Serverless режиме.
// Новая логика использует его только в блоке !process.env.VERCEL.
// Я также убрал строгую проверку Email на Шаге 3, так как вы запросили сбор разных контактов.